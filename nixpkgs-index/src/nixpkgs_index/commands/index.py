"""Index subcommand for nixpkgs package indexing."""
...
from .utils import setup_logging, parse_interval
...
def index(
    ...
    index_obj = Index.load(output_path)
    logger.info(
        f"Starting index state: {sum(len(pkg.versions) for pkg in index_obj.pkgs.values())} total versions"
    )
    for pkg_name, pkg_index in index_obj.pkgs.items():
        ...
        for pkg_name, pkg_config in config_obj.pkgs.items():
            for attribute in pkg_config.nixpkgs_attributes:
                ...
                if version:
                    ...
                    updated = index_obj.update_version(
                        pkg_name,
                        version,
                        commit.sha,
